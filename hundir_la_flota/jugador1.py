...
def on_message(mqttc, userdata, msg):   
    ...
    pygame.init()
    if mensaje == 'jugador1, se ha ido fuera de rango': 
        data['lanzar'] = True
        ...
        if len(mensaje.split(','))==3:
            color = (0,0,120) #AZUL
            jugadorYagua, casilla = mensaje.split(':')
            jugador, agua = jugadorYagua.split(',')
        ...
        if jugador == 'jugador1':
            ...
            c = c +22
        tablero[f][c] = color
        pantalla = pygame.display.set_mode([1060, 550]) #DIMENSION DE LA PANTALLA
        pantalla.blit(oceano,(0,0))
        pygame.display.set_caption("TABLERO DEL JUGADOR 1")
        letra = pygame.font.SysFont("PHOSPATE",30)
        texto = letra.render("JUGADOR 1                                                                             JUGADOR 2",True,(0,0,0))
        ...
        for filas in range(20): #dimension del tablero a lo alto
            for columnas in range(42): #dimension del tablero a lo ancho
...
k = casillas_de_barcos(fichero)[1]
...
data={'tablero':tablero,'lanzar':False}
...
while True: 
    if data['lanzar']:
        ...
        if fila >= 20 or columna >= 20:
            m='jugador1, se ha ido fuera de rango'
            mqttc.publish('clients/tablero',m)
